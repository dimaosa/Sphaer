...
class Countries {
    ...
    fileprivate(set) static var countries: [Country] = {
    ...
    class func countryFromPhoneExtension(_ phoneExtension: String) -> Country {
        let phoneExtension = phoneExtension.replacingOccurrences(of: "+", with: "")
        
        if let countryIndex = countries.index(where: {$0.isMain && phoneExtension == $0.phoneExtension}) {
    ...
    class func countryFromCountryCode(_ countryCode: String) -> Country {
        if let countryIndex = countries.index(where: {countryCode == $0.countryCode}) {
            return countries[countryIndex]
        } else {
            return Country.emptyCountry
        }
